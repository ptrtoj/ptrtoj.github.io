...
categories: [Linux]
tags: [linux, korean, keyboard, 한글, 입력, IME, ibus, fctix, xim]
---

`fcitx`, `scim` 등도 잘 작동 하는 것으로 알고 있습니다만, 저는 리눅스를 처음 접했을 때부터 지금까지 `ibus`로 문제없이 사용중이므로 가이드는 `ibus` 기준으로 작성합니다.
...
`ibus`를 사용하기 위해서는 `/etc/envrionment`{: .filepath } 파일 수정이 필요합니다.
...
>
> `/etc/environment`{: .filepath}에 입력한 변수들을 설정하라는 알림이 뜰텐데 `Yes` 하시면 됩니다.
> 그리고 나면, 아래와 같은 화면이 표시됩니다.
{: prompt-warning }

Input Method을 클릭합니다.

![ibus screen 1](/assets/img/posts/korean-1.png)
...
![ibus screen 2](/assets/img/posts/korean-2.png)

언어 목록이 뜹니다. 하단의 세로로 된 **⁞** 를 클릭합니다.

![ibus screen 3](/assets/img/posts/korean-3.png)

입력창에 보시는 것처럼 korea를 입력합니다.

![ibus screen 4](/assets/img/posts/korean-4.png)

ibus-hangul이 제대로 설치되었다면 **태극 문양의 Hangul**이 표시되어야 합니다.

![ibus screen 5](/assets/img/posts/korean-5.png)

해당 Hangul을 클릭하고 Add를 눌러 나옵니다.

![ibus screen 6](/assets/img/posts/korean-6.png)

이제 입력기는 추가되었습니다.

우측 Alt 혹은 한/영키를 이용해 입력기를 변경하기 위해서 Preferences 단추를 누릅니다.

![ibus screen 7](/assets/img/posts/korean-7.png)
...
![ibus screen 8](/assets/img/posts/korean-8.png)
...
![ibus screen 9](/assets/img/posts/korean-9.png)
...
윈도우 매니저를 활용하시는 분들은 `.xinitrc`{: .filepath}나 본인 윈도우 매니저의 autostart 설정을 통해 `$ibus-daemon -drxR`을 추가합니다.
